...
namespace NArrange.Tests.CSharp
{
    ...
    public class CSharp6FeatureTests
    {
        ...
        /// <summary>
        /// Will assert for each file that it is infact compilable, then format it using NArrange and then check that it is still compilable.
        /// Important as some C# 6 features break the current NArrange implementation (formatting would create broken code)
        /// </summary>
        [Test]
        public void AssertCodeUsingCSharp6CanCompileEvenWhenFormatted()
        {
            var csharp6Files = new[]
            {
                "AutoPropertyInitializers.cs",
                "ExpressionBodies.cs",
                "NullConditionalOperator.cs",
                "StaticExtensionMethods.cs",
                "StaticNamespaces.cs"
            };
            foreach (var name in csharp6Files)
            {
                var resourceName = "CSharp_6_0_Features." + name;
                ...
                try
                {
                    ...
                    currentContent.Should().NotBe(newContent, "because regions where missing and NArrange always adds them for us.");
                    Compile(tempFile).Should().BeTrue();
                }
                finally
                {
